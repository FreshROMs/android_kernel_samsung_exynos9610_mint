...
enum {
	...
	BIG_MIN_FREQ,
	BIG_MAX_FREQ,
#if defined(CONFIG_AROGS_THREE_CLUSTERS)
	MID_MIN_FREQ,
	MID_MAX_FREQ,
#endif
	LIT_MIN_FREQ,
...
#else
	pr_info("%s name:%s, BIG_MIN=%d, BIG_MAX=%d, LIT_MIN=%d, LIT_MAX=%d, MIF=%d, INT=%d\n",
		__func__, cname, big_min_freq, big_max_freq, lit_min_freq, lit_max_freq, mif_freq, int_freq);
#endif
}
