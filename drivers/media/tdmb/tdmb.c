...
static void tdmb_set_config_poweron(void)
{
	...
	if (dt_pdata->tdmb_ant_sw > 0) {
		rc = gpio_request(dt_pdata->tdmb_ant_sw, "gpio_tdmb_ant_sw");
		if (rc < 0) {
			DPRINTK("%s: gpio_tdmb_ant_sw request failed (%d)\n", __func__, rc);
			dt_pdata->tdmb_ant_gpio_req = false;
		} else {
			dt_pdata->tdmb_ant_gpio_req = true;
		}
	}
	...
	if (dt_pdata->tdmb_use_irq) {
		...
		if (rc < 0) {
			...
			if (dt_pdata->tdmb_ant_gpio_req && dt_pdata->tdmb_ant_sw > 0)
				gpio_free(dt_pdata->tdmb_ant_sw);
			return;
		}
	}
...
static void tdmb_gpio_on(void)
{
	...
	if (dt_pdata->tdmb_ant_sw > 0)
		gpio_direction_output(dt_pdata->tdmb_ant_sw, dt_pdata->tdmb_ant_sel);
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	pdata->tdmb_ant_sw = of_get_named_gpio(dev->of_node, "tdmb_ant_switch", 0);
	if (!gpio_is_valid(pdata->tdmb_ant_sw)) {
		DPRINTK("Failed to get is valid tdmb_ant_sw\n");
	} else {
		pdata->tdmb_ant_gpio_req =  false;

		if (of_property_read_u32(dev->of_node, "tdmb_ant_sel",
								&pdata->tdmb_ant_sel)) {
			DPRINTK("Failed to get tdmb_ant_sel\n");
		}
	}
