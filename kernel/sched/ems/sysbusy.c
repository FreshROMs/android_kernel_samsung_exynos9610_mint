...
static int sysbusy_find_fastest_cpu(struct tp_env *env)
{
	...
	cpumask_and(&mask, cpu_fastest_mask(), tsk_cpus_allowed(env->p));
	if (cpumask_empty(&mask))
		cpu = cpumask_last(tsk_cpus_allowed(env->p));
	else
...
int sysbusy_schedule(struct tp_env *env)
{
	...
	case SYSBUSY_STATE3:
		if (is_heavy_task_util(env->task_util)) {
			if (cpumask_test_cpu(task_cpu(env->p), tsk_cpus_allowed(env->p)))
